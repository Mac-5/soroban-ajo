...
fn test_contribution_late() {
    ...
    env.ledger()
        .with_mut(|li| li.timestamp += cycle_duration + 3600); // 1 day + 1 hour
...
// ============================================================================
// EDGE CASE TESTS FOR CONTRIBUTION FLOW
// ============================================================================

#[test]
fn test_duplicate_contribution_same_cycle_multiple_members() {
    let (env, client, creator, member2, member3) = setup_test_env();
    ...
    // All members contribute once
    client.contribute(&creator, &group_id);
    client.contribute(&member2, &group_id);
    client.contribute(&member3, &group_id);

    // Each member tries to contribute again in same cycle
    assert_eq!(
        client.try_contribute(&creator, &group_id),
        Err(Ok(AjoError::AlreadyContributed))
    );
    assert_eq!(
        client.try_contribute(&member2, &group_id),
        Err(Ok(AjoError::AlreadyContributed))
    );
    assert_eq!(
        client.try_contribute(&member3, &group_id),
        Err(Ok(AjoError::AlreadyContributed))
    );
}

#[test]
fn test_duplicate_contribution_after_partial_contributions() {
    let (_, client, creator, member2, member3) = setup_test_env();
    ...
    // Creator tries to contribute again before others
    assert_eq!(
        client.try_contribute(&creator, &group_id),
        Err(Ok(AjoError::AlreadyContributed))
    );
...
#[test]
fn test_contribution_after_all_cycles_complete() {
    let (_, client, creator, member2, _) = setup_test_env();

    let group_id = client.create_group(&creator, &100_000_000i128, &604_800u64, &2u32);
    client.join_group(&member2, &group_id);

    // Complete all cycles (2 members = 2 cycles)
    for _ in 0..2 {
        client.contribute(&creator, &group_id);
        client.contribute(&member2, &group_id);
        client.execute_payout(&group_id);
    ...
    // Both members try to contribute after completion
    assert_eq!(
        client.try_contribute(&creator, &group_id),
        Err(Ok(AjoError::GroupComplete))
    );
    assert_eq!(
        client.try_contribute(&member2, &group_id),
        Err(Ok(AjoError::GroupComplete))
...
#[test]
fn test_contribution_after_completion_with_max_members() {
    let (env, client, creator, member2, member3) = setup_test_env();
    ...
    // Try contribution from existing members after completion
    assert_eq!(
        client.try_contribute(&creator, &group_id),
        Err(Ok(AjoError::GroupComplete))
    );
...
#[test]
fn test_zero_contribution_on_existing_group() {
    let (_, client, creator, _, _) = setup_test_env();

    // Create group with valid contribution amount
    let group_id = client.create_group(&creator, &100_000_000i128, &604_800u64, &3u32);

    // Verify group exists with non-zero contribution
    let group = client.get_group(&group_id);
    assert_eq!(group.contribution_amount, 100_000_000i128);
    assert!(group.contribution_amount > 0);
}

#[test]
fn test_negative_contribution_boundary() {
    let (_, client, creator, _, _) = setup_test_env();

    // Test various negative values
    assert_eq!(
        client.try_create_group(&creator, &-1i128, &604_800u64, &3u32),
        Err(Ok(AjoError::ContributionAmountNegative))
    );
    assert_eq!(
        client.try_create_group(&creator, &-100i128, &604_800u64, &3u32),
        Err(Ok(AjoError::ContributionAmountNegative))
    );
    assert_eq!(
        client.try_create_group(&creator, &-1_000_000i128, &604_800u64, &3u32),
        Err(Ok(AjoError::ContributionAmountNegative))
    );
}

#[test]
fn test_zero_contribution_boundary() {
    let (_, client, creator, _, _) = setup_test_env();

    // Exactly zero should fail
    assert_eq!(
        client.try_create_group(&creator, &0i128, &604_800u64, &3u32),
        Err(Ok(AjoError::ContributionAmountZero))
    );

    // Positive value should succeed
    let result = client.try_create_group(&creator, &1i128, &604_800u64, &3u32);
    assert!(result.is_ok());
}

#[test]
fn test_duplicate_contribution_across_different_groups() {
    let (_, client, creator, member2, _) = setup_test_env();

    // Create two separate groups
    let group_id1 = client.create_group(&creator, &100_000_000i128, &604_800u64, &3u32);
    let group_id2 = client.create_group(&member2, &200_000_000i128, &604_800u64, &3u32);

    // Creator joins both groups
    client.join_group(&creator, &group_id2);

    // Creator contributes to group 1
    client.contribute(&creator, &group_id1);

    // Creator should still be able to contribute to group 2
    client.contribute(&creator, &group_id2);

    // But not again to group 1
    assert_eq!(
        client.try_contribute(&creator, &group_id1),
        Err(Ok(AjoError::AlreadyContributed))
    );
}

#[test]
fn test_contribution_after_completion_then_new_group() {
    let (_, client, creator, member2, _) = setup_test_env();

    // Create and complete first group
    let group_id1 = client.create_group(&creator, &100_000_000i128, &604_800u64, &2u32);
    client.join_group(&member2, &group_id1);

    for _ in 0..2 {
        client.contribute(&creator, &group_id1);
        client.contribute(&member2, &group_id1);
        client.execute_payout(&group_id1);
    }

    // Verify first group is complete
    ...
    let group_id2 = client.create_group(&creator, &100_000_000i128, &604_800u64, &2u32);
    client.join_group(&member2, &group_id2);

    // Should be able to contribute to new group
    client.contribute(&creator, &group_id2);
    client.contribute(&member2, &group_id2);
}
